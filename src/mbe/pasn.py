...
def spikes_per_input(neuron, x: torch.Tensor) -> float:
    """Mean spikes per input element for either neuron type (unified cost meter)."""
    if isinstance(neuron, PASNNeuron):
        return neuron.mean_spikes(x)
    return neuron.firing_rate(x) * neuron.cfg.n_basis * neuron.cfg.n_steps
